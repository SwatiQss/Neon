...
`;
  ...
  static async getRound(id) {
    const sql =
      "SELECT e.*,COALESCE(r.occurrence_count, 0) AS occurrence_count FROM events e LEFT JOIN ( SELECT event_id, COUNT(*) AS occurrence_count FROM reviews GROUP BY event_id) r ON e.id = r.event_id where e.id=$1 ";
  ...
  static async getItenary() {
    const sql =
      "SELECT e.*,COALESCE(r.occurrence_count, 0) AS occurrence_count FROM events e LEFT JOIN ( SELECT event_id, COUNT(*) AS occurrence_count FROM reviews GROUP BY event_id) r ON e.id = r.event_id where e.status='active' ";
